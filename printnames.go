// A modified version of the original, which is available in the docker source code at
// https://github.com/dotcloud/docker/blob/master/pkg/namesgenerator/names-generator.go
// Reused here under the terms of the Apache License
...
func PrintName() {
  fmt.Println("- name:", GetRandomName())
}

func main() {
  if len(os.Args) != 2 {
    log.Fatal("Must specify number of names required as only argument")
  }
  required_names, err := strconv.Atoi(os.Args[1])
  if err != nil {
    log.Fatal("Number of names argument must be an integer")
  }
  PrintYamlHeader()
  count := 1
  for count < required_names+1 {
    count += 1
    PrintName()
  }
}
